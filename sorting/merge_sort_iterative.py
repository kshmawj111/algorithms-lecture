def iterative_merge(A, left, mid, right, verbose=False):
    N = right - left + 1
    n1 = mid-left + 1
    n2 = right - mid
...
def merge_sort_iter(ary: list, verbose=False):
    size, i = 1, 0
    length = len(ary)
    ...
    while size < length:
        ...
        while left < length: # mergesort 하고자 하는 subarray의 범위를 계산함
            ...
            iterative_merge(ary, left, mid, right, verbose=verbose)
